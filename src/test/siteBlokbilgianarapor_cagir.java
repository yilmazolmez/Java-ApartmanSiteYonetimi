...
public class siteBlokbilgianarapor_cagir {
	...
	public void rapor(String sayi) {
		...
		try 
		{
			...
			String listele="Select * From siteblokbilgi where BlokID='"+sayi+"'";
			...
			rs1=mystat1.executeQuery(listele);
				...
				{	
					...
					PdfWriter pdfWriter = PdfWriter.getInstance(document, new FileOutputStream(path+"\\siteblokbilgi.pdf"));
					document.open();
					//ekrana yazýlar basýlýyor
					document.add(new Paragraph("SECILEN BLOK BILGI SATIRI BILGILERI :"));
					
		         //Liste yöntemiyle veri ekleme
			         List list = new List(List.UNORDERED);
			            list.add(new ListItem("Blok id="+rs1.getString("BlokID")));
			            list.add(new ListItem("Blok Adi/No ="+rs1.getString("BlokAdi")));
			            list.add(new ListItem("Blok Adresi="+rs1.getString("Blokadres")));
			            list.add(new ListItem("Blok Sorumlusu="+rs1.getString("Bloksorumlusu")));
			            list.add(new ListItem("---------------------------------------------------------------------------------------------------------"));
			            list.add(new ListItem("Blok Dis Kapi No="+rs1.getString("Blokdiskapino")));
			            list.add(new ListItem("Blok Aciklama="+rs1.getString("Blokaciklama")));
			            list.add(new ListItem("---------------------------------------------------------------------------------------------------------"));
			       ...
			       // String FILE = "C:\\Users\\yýlmaz\\eclipse-workspace\\Deneme\\avukatbilgi.pdf";
			        String FILE = path+"\\siteblokbilgi.pdf";
					try 
					{
