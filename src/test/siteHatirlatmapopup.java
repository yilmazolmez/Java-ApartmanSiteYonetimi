...
public class siteHatirlatmapopup extends JDialog {
	...
	public void clock()
	{
		...
		lblNewLabel.setBounds(13, 11, 1094, 37);
	...
	public siteHatirlatmapopup() {
		setResizable(false);
		setTitle("Site Otomasyonu - POP UP Hatirlatma Ekrani");
		setModal(true);
		setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);
		setBounds(100, 100, 1137, 558);
		...
		scrollPane.setBounds(13, 91, 1094, 353);
...
//					String listele="Select A.Aciklama,A.hatirlat,A.hakedis,B.Blokbolumadi From sitebolumcarihareket A,sitebolumbilgi B where A.Blokbolumadi=B.Blokbolumadi and A.hatirlat>=DATE(NOW())  UNION ALL Select C.Aciklama,C.hatirlat,C.hakedis,D.MuvekkilAdi From muvekkilhukukihareket C,muvekkilbilgi D where C.MuvekkilTC=D.MuvekkilTC and C.hatirlat>=DATE(NOW()) UNION ALL Select E.Aciklama,E.hatirlat,E.hakedis,F.MuvekkilAdi From muvekkilozelhareket E,muvekkilbilgi F where E.MuvekkilTC=F.MuvekkilTC and E.hatirlat>=DATE(NOW()) UNION ALL Select G.Aciklama,G.hatirlat,G.hakedis,H.MuvekkilAdi From muvekkilcarihareket G,muvekkilbilgi H where G.MuvekkilTC=H.MuvekkilTC and G.hatirlat>=DATE(NOW()) order by hatirlat asc ";
					String listele="Select A.Blokbolumadi,A.Aciklama,A.hatirlat,A.hakedis From sitebolumcarihareket A where A.hatirlat>=DATE(NOW()) UNION ALL Select C.personeladi,C.Aciklama,C.hatirlat,C.hakedis From siteortakcarihareket C where C.hatirlat>=DATE(NOW()) UNION ALL Select B.personeladsoyad,B.Aciklama,B.hatirlat,B.hakedis From sitepersonelcarihareket B where B.hatirlat>=DATE(NOW()) UNION ALL Select D.personeladisoyadi,D.Aciklama,D.hatirlat,D.hakedis From siteajanda D where D.hatirlat>=DATE(NOW()) UNION ALL Select E.sikayetedensakinbilgi,E.Aciklama,E.hatirlat,E.hakedis From sitesikayetbilgihareket E where E.hatirlat>=DATE(NOW()) order by hatirlat asc ";
...
//					String listele="Select * From burobilgi";
					...
					while(rs1.next())
					{
						satirlar[0]=rs1.getString("Blokbolumadi");
		...
		btnNewButton.setBounds(13, 455, 1094, 53);
